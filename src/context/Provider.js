...
function Provider({ children }) {
  ...
  const [columnFilterArrayValues, setColumnFilterValue] = useState([
    "population",
    "orbital_period",
    "diameter",
    "rotation_period",
  ...
  const restoreDataBeforeExclusion = async (item) => {
    ...
    const newdata = attFilterSave.map((itemsalvo, index) =>
      dataComplete?.filter((y) => {
  ...
  const sortDataPopulation = () => {
    ...
    if (typeOrder === "ASC") {
      const dataSortNumber = dataWithoutUnknown.sort(
        (a, b) => a[order] - b[order]
      );
